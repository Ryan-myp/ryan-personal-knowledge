...
// CreativeAsset represents an ad creative generated by AI.
...
// CreativeGenerator generates ad creatives using AI models.
type CreativeGenerator struct {
	mu          sync.RWMutex
	generators  map[string]CreativeModel
	pending     chan *GenerationRequest
	results     chan *CreativeAsset
}
...
type CreativeModel interface {
	Name() string
	Generate(prompt string, params map[string]interface{}) (*CreativeAsset, error)
}

func NewCreativeGenerator(numWorkers int) *CreativeGenerator {
	cg := &CreativeGenerator{
		generators: make(map[string]CreativeModel),
		pending:    make(chan *GenerationRequest, 100),
		results:    make(chan *CreativeAsset, 100),
	}
...
func (cg *CreativeGenerator) worker(id int) {
	for req := range cg.pending {
		model, exists := cg.generators[req.Type]
		if !exists {
			continue
		}

		asset, err := model.Generate(req.Prompt, req.Params)
		if err != nil {
			continue
		}
...
// RegisterModel registers a creative generation model.
func (cg *CreativeGenerator) RegisterModel(model CreativeModel) {
	cg.mu.Lock()
	defer cg.mu.Unlock()
	cg.generators[model.Name()] = model
}

// GenerateAsync submits a generation request.
func (cg *CreativeGenerator) GenerateAsync(req *GenerationRequest) {
	cg.pending <- req
}

// GetResult waits for a generation result.
func (cg *CreativeGenerator) GetResult(requestID string) (*CreativeAsset, error) {
	for asset := range cg.results {
		if asset.Metadata["request_id"] == requestID {
			return asset, nil
		}
	}
	return nil, fmt.Errorf("result not found for %s", requestID)
}
```
...
### 问题 1：为什么 CreativeGenerator 使用 channel 而非直接调用模型？
...
Channel 的优势：
1. **异步处理**：不阻塞调用方
2. **流量控制**：pending channel 容量限制并发数
3. **解耦**：生成请求和结果处理分离
4. **可扩展**：可以轻松增加 worker 数量

直接调用会导致同步等待，影响用户体验。

</details>

### 问题 2：为什么 RegisterModel 用 Lock 而 worker 用 range channel？

<details>
<summary>查看答案</summary>

RegisterModel 修改 generators map（写操作），需要排他锁。
worker 通过 channel 接收请求（读操作），不需要额外锁。

这种设计将共享状态（generators）的并发访问最小化，提高性能。

</details>

### 问题 3：AI 创意生成的批量处理策略是什么？

<details>
<summary>查看答案</summary>

1. **并行生成**：多个 worker 同时处理不同请求
2. **批处理**：相同类型的请求合并处理（如批量图片生成）
3. **优先级队列**：高预算 Campaign 优先处理
4. **缓存复用**：相似 prompt 的结果缓存
