...
1. **百分比显示**：已完成模块数 / 总模块数 * 100%，直观展示学习进度
2. **进度条渲染**：ASCII 条形图 ███░░ 60% 用于终端展示，美观且一目了然
3. **天数预估**：根据剩余模块数和每天学习速率估算 remaining days
4. **里程碑标记**：关键节点（如完成 Go 基础、构建第一个 Agent）突出显示给予成就感
5. **时间序列图表**：按天绘制 completed count 折线图，可视化学习曲线的平稳性或波动性
</details>
