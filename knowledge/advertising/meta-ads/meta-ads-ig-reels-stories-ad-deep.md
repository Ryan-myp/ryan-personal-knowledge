...
## 十四、补充：Stories 与 Reels 在"贴纸与 CTA"上的落地差异

因为 Stories 与 Reels 的 UI 与用户行为不同，同一种贴纸/CTA 的落地方式也不同：

| 元素 | Stories | Reels |
|------|---------|-------|
| 上滑虚线 "查看更多" | 经典，指向落地页/结账 | 已移除旧的"上滑"模式，改用主页链接/评论置顶 |
| 投票/滑杆贴纸 | 常见，短促参与 | 常见，评论区/贴纸可引导 |
| Link 贴纸 | 上滑即可跳转 | 需主页/简介链接，或评论区置顶链接，或 Spark Ads |
| 底部 CTA | 原生 "TAP TO" 条 | 原生 CTA 按钮（Learn More 等） |
| 评论互动 | 弱（Stories 无评论区） | 强（Reels 评论区是核心互动） |

**因此撰写 CTA 时的取舍：**
- Stories：走"**上滑/点击底部 TAP TO**"，文案短、行动词强（"点我领券""滑上去看"）；
- Reels：走"**评论区扣1 / 主页链接 / 底部 CTA**"，文案可加"评论区告诉我"引导互动。

---

## 十五、补充：竖版素材的"跨平台一致性"思考（多平台投放时的取舍）

若同一竖版素材还要放到 TikTok / YouTube Shorts 等平台，注意：
- 比例（9:16）通用，但**安全区、UI、字幕位置、时长上限、贴纸生态各有差异**；
- 建议为每平台保留一份"含该平台 UI 参考线"的母版，避免一次性裁切到处用；
- 竖版素材的"叙事结构"（首 3 秒钩子、无声可懂）是**跨平台通识**，可复用，
  但**贴纸/CTA/版位边界**必须按平台重做。

> 一句话：**故事结构可复用，平台细节要重做。**

---

## 附录 D：本深文档相对其他文档的"分工界定"

| 文档 | 侧重 |
|------|------|
| `meta-ads-objectives-creatives-deep.md` | 通用广告目标 + 通用创意格式（单图/视频/轮播...） |
| **本文档** | **IG Reels & Stories 竖版视频专属**：9:16 规范/安全区/字幕/贴纸/版位/完播-TTR 调优/BeReal 风生产管线 |
| `meta-ads-advantage-plus-full-deep.md` | Advantage+ 出价/预算/自动版位系统级深讲 |
| `meta-ads-targeting-advantage-deep.md` | 定向与受众深讲 |

阅读提示：本文已经避免重复通用创意规范，聚焦竖版这一垂直场景；若需通用目标/格式知识，
请回溯上述对应文档。
