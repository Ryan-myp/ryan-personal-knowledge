...
### 2.15 与手动系列对比：什么时候值得切到 ASC

很多团队犹豫「要不要从手动切到 ASC」。下表是决策依据（经验性判断）：

| 判断维度 | 适合 ASC | 倾向保留手动 |
|----------|----------|--------------|
| 转化事件量 | 充足（每周几十+） | 稀疏（每周个位数） |
| 素材储备 | 多且语义多样 | 少且同质 |
| 对受众控制的需求 | 低，愿意交给模型 | 高（合规/品牌严格） |
| 预算规模 | 中等及以上 | 极小 |
| 目标 | 放量、降 CPA | 精细测试、隔离变量 |
| 团队实验能力 | 振荡中常用 | 需审慎 |

**黄金三角判断**：当「转化信号充足 + 素材多样 + 愿意放权」三者同时成立时，ASC 的收益杠杆最高；缺任意一环，ASC 的优势都会被压缩。
...
| 症状 | 可能原因 | 排查动作 |
|------|----------|----------|
| 安装成本虚高 | App 事件与广告点击关联失败（SDK 初始化时机） | 检查 click_id / event source 链路 |
| 付费事件学不到 | MMP 归因口径与 Meta 冲突 | 对齐归因窗口、检查 S2S 回传 |
| 只买得到「羊毛党」 | 优化目标过于靠前（如 install） | 切到 App Events / ROAS 目标 |
| 广告被拒 | 素材含敏感/误导内容 | 检查素材审核规则 |
| 数据不一致 | SDK 与 MMP 双份事件未去重 | 明确事件源，启用去重 |

### 4.19 关于「测试与放量」的完整方法论
...
> 为帮助读者把前文原理落到日常操作，本节给出三个代表性的场景复盘。案例为脱敏后的业务经验总结，不代表任何公司真实数据。

### 6.1 场景 A：从 DPA 迁移到 ASC 的完整过程

**背景**：某跨境电商（客单价 $40，主销北美），原来用 DPA + 手动受众跑转化，ROAS 稳定在 2.0-2.4，但放量期遇到瓶颈（扩量就掉 ROAS）。
...
│   ├── 保留原 DPA（对照）
│   └── 新建 ASC（预算 = DPA 的 50%）
│       素材：从原素材池选出语义多样的 12 条（3 视频 + 3 图 + 6 文案）
│
第 2-3 周：学习期
│   ├── ASC 前 3 天 CPA 高于 DPA 约 40%（符合预期，探索阶段）
│   ├── 第 5 天起 CPA 快速下行
│   └── 第 7 天 ROAS 追平 DPA（2.3）
│
第 4 周：放量
│   ├── ASC 预算 +30%，ROAS 保持 2.2
│   ├── 再 +25%，观察到 CPA 小幅回升 → 暂停在稳定点
│   └── DPA 缩预算用于再营销（加购人群）
│
第 8 周：结论
└── ASC 渠道 ROAS 2.3 vs DPA 2.0，量达到 DPA 的 2.5 倍
```

**复盘要点**：

- 不要第一天就删 DPA：并行 2-4 周做对照，数据说话；
- 学习期前 3 天的「CPA 偏高」是正常现象，凭第一天数据开枪是常见误操作；
- 放量必须阶梯化，模型和素材都需要时间消化新预算。

### 6.2 场景 B：CAPI 没配好导致 ASC 一直不起量

**背景**：某快消品牌，Pixel 事件完整，但 ASC 从不上量，日预算 60% 花不出去。
...
**根因**：浏览器像素受广告拦截/隐私政策影响，转化事件大量丢失，模型「看不到转化」，于是不敢放量。

**修复**：

```python
# 1. 接入 CAPI，补齐服务端 Purchase 事件
client.meta_send_capi(
    pixel_id,
    event_name="Purchase",
    event_time=ts,
    user_data={"em": email_hash, "ph": phone_hash},
    custom_data={"currency": "USD", "value": 49.99, "event_id": order_id},
)

# 2. 用 event_id 去重，避免与 Pixel 双写
# 3. 提升信号：把 AddToCart / InitiateCheckout 也一并回传
```

接入后 2 周内，转化事件量翻倍，ASC 开始正常花预算并收敛，CPA 下降 25%。

**经验**：遇到「不起量」先查数据，再查素材与预算，顺序不能反。

### 6.3 场景场景 C：多国投放时「按国拆 AdSet」的教训

**背景**：某工具类产品同时投英国与德国，团队按国各建一个 ASC（每个 AdSet 单国），预算各 $200/天。

**结果**：德国站点转化事件稀疏，该 campaign 长期处于探索态，CPA 是英国的 3 倍，ROAS 甚至 < 1。

**调整**：

```
调整前（错误）
├── ASC-UK（$200/天，信号充足，学得好）
└── ASC-DE（$200/天，信号稀疏，学不动）
│
调整后（正确）
└── ASC-EU（$400/天，英国+德国一起，模型自动分配）
    ├── 模型把预算优先给转化密度高的英国
    └── 德国流量从「强行买量」变为「探索补充」
```

合并后第 3 周，总 CPA 比原来两套加起来还低 15%，德国仍有少量流量覆盖但不再拖垮整体。

**经验**：信号弱的市场不要强行独立 campaign，先让模型在更大池子里自主分配，看够了再考虑是否拆。
...
Advantage+ 通用操作守则
│
├── 1. 数据先行：每次新建/放量前核对 Pixel/CAPI 与转化事件
├── 2. 学习期铁律：5-7 天内不乱动，动一次重新计时
├── 3. 素材是燃料：语义多样 > 数量堆砌，上线前一次给足
├── 4. 放量阶梯化：+20-30% 一步，看 3-5 天再动
├── 5. 对照先行：迁移/换型都对跑 2-4 周再下结论
├── 6. 用 Insights 定位：先问「数据准不准」，再问「策略对不对」
└── 7. 制度化记录：每次操作记日志（谁、何时、改了什么、7 天后指标）
```
