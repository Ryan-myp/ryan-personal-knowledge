...
1. **规则爆炸**：审核规则数量过多导致匹配复杂度 O(n) 线性增长
2. **并发瓶颈**：单线程审核成为串行瓶颈，无法利用多核 CPU
3. **特征缺失**：图像/视频审核缺乏预计算的特征缓存，每次都要重算
4. **误报率高**：AI 初审准确率不足导致大量无效转人工，降低整体 throughput
5. **解决方案**：引入 Redis 缓存审核结果、使用 worker 池并行处理、定期重训练模型降低误报率
