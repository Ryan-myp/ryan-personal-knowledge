...
## 附录 E：自动出价内部机制深化（模型与校准）

前面 2.2 节概述了 Auto-bidding 的"预测→出价→pacing"闭环。本节深入自动出价背后的模型与校准细节，帮助理解为什么"同样设置目标，不同账户结果差异巨大"。

### E.1 pCVR / pCTR 的预测链路

DV360 的自动出价预测并非单个模型，而是一条多阶段链路：

```
广告请求到达 (bid request)
   │
   ▼
[特征工程] 上下文/用户/广告位/历史 特征向量
   │
   ▼
[pCTR 模型] ──► 预估点击概率
   │
   ▼
[pCVR 模型] ──► 预估转化概率 (需转化信号)
   │
   ▼
[价值/校准] ──► 结合目标 (CPA/ROAS) 计算期望价值
   │
   ▼
[出价决策] ──► pacing/floor/ceiling 约束后出价
```

**要点**：
- pCVR 比 pCTR 更"远"、更难预测，需要更多转化数据支撑；
- 在没有转化回传的纯品牌 Line Item 上，pCVR 无法建模，自动出价只能优化点击/可见性等代理指标；
- 转化稀疏（低频转化）会显著拉长学习期、提高方差。

### E.2 校准（Calibration）：预测与现实的偏差

模型预测的系统性偏差是自动出价失效的头号隐藏原因：

| 偏差类型 | 现象 | 后果 |
|---------|------|------|
| pCVR 系统性高估 | 预测 2% 实际 1% | 出价虚高，成本超标 |
| pCVR 系统性低估 | 预测 1% 实际 2% | 出价保守，量不足 |
| 时区/时段偏差 | 高峰低估低谷高估 | 预算分配失衡 |
| 新流量偏差 | 新受众无历史 | 冷启动出价不稳 |
...
**踩坑实录**：
> 某团队上线 Target ROAS 后两周 ROAS 始终低于目标，排查发现转化回传延迟 3 天（归因窗口内的转化被重复/延迟回传），导致模型"看到"的转化滞后，pCVR 校准失真。修复回传时序后 ROAS 回归正常。**教训：转化回传的及时性与去重直接决定自动出价模型的质量。**

### E.3 自动出价的冷启动（Cold Start）

新 Line Item / 新定向 / 新创意上线时，模型没有该组合的历史信号，处于冷启动：

```
冷启动阶段特征:
  - 模型用"相似组合"的先验做初始估计 (transfer learning)
  - 探索式出价: 以一定概率试探不同出价区间
  - 表现方差大, 早期 CPA/成本偏高
...
**冷启动加速手段**：
1. 复用同品类成熟 Line Item 的历史，让模型 warm-start；
2. 定向不要过窄，给模型足够样本；
3. 初期用略宽松的目标，稳定后再收紧；
4. 避免在冷启动期叠加 Surge / 频繁改动。

### E.4 自动出价与频控/频次的协同

自动出价优化单次展示价值，但可能与 frequency cap 产生张力：

- 高价值用户被频繁出价 → 引擎可能对同一用户多次出价 → 频控拦下 → 出价"浪费"在无法成交的机会；
- 解决方案：出价引擎与频控协同考虑"该用户剩余可展示次数"。

```
协同逻辑:
  对用户 u 在机会 o 上:
    remaining_cap = cap - 已展示(u)
    if remaining_cap <= 0:
        放弃该机会 (不浪费出价)
    else:
        # 剩余次数越少, 机会越稀缺, 越值得出价
        bid = base_bid × (剩余频次越小 → 权重越高, 但受 cap 硬约束)
```

这一层通常由 DV360 自动处理，但理解它有助于解释"为什么有时高价值用户不出价"。

### E.5 自动出价的缺陷与不适合场景（诚实边界）

自动出价并非万能，以下场景它往往不如手动/或需要特殊处理：

| 不适合场景 | 原因 | 建议 |
|-----------|------|------|
| 转化数据极少(<每周30) | 模型学不动 | 先手动或 Maximize 收集数据 |
| 转化回传混乱 | 信号失真 | 先修回传 |
| 极短排期(<3天) | 学习期都过不完 | 手动出价或加速冷启动 |
| 纯投放狭窄保量 PG | 价格已固定 | 手动固定 CPM |
| 预算极小 | 样本不足以建模 | 提升预算或合并 |
| 高度季节性突发 | 历史无先验 | 谨慎使用, 结合 Surge |

---

## 附录 F：出价策略横向深度对比

本节用表格系统对比 DV360 主要出价策略在多个维度上的表现，作为选型的快速参考。

### F.1 策略 × 维度总览

| 维度 | 手动CPM | 手动CPC | 手动CPV | Maximize Clicks | Maximize Conv | Target CPA | Target ROAS | Viewable CPM | oCPM |
|------|--------|--------|--------|----------------|---------------|-----------|-------------|-------------|------|
| 是否需要转化数据 | 否 | 否 | 否 | 否 | 高 | 高 | 高 | 中 | 高 |
| 是否需要价值回传 | 否 | 否 | 否 | 否 | 否 | 否 | 是 | 否 | 否 |
| 优化目标自主性 | 无 | 无 | 无 | 点击量 | 转化量 | 成本内转化 | 回报内收入 | 可见展示 | 成本内转化 |
| 是否动态出价 | 否 | 否 | 否 | 是 | 是 | 是 | 是 | 是 | 是 |
| 学习期 | 无 | 无 | 无 | 短 | 中 | 中长 | 中长 | 中 | 中长 |
| 成本可控性 | 高 | 高 | 高 | 中 | 中低 | 中 | 中 | 中 | 中 |
| 拿量能力 | 低-中 | 低 | 低 | 高 | 高 | 中高 | 中高 | 中 | 中高 |
| 适用层级 | 品牌/保量 | 点击 | 视频 | 流量 | 效果 | 效果 | 电商 | 品牌 | 效果 |
| 与Surge兼容度 | 高 | 高 | 高 | 中 | 低 | 低 | 低 | 中 | 低 |

### F.2 关键对比：Target CPA vs Target ROAS vs Maximize

| 对比项 | Target CPA | Target ROAS | Maximize Conversions |
|-------|-----------|-------------|---------------------|
| 设定内容 | 目标CPA + 预算 | 目标ROAS + 预算 | 预算 |
| 目标方向 | 控制单次转化成本 | 控制回报率 | 最大化总转化量 |
| 核心输入 | pCVR | pCVR × conversion_value | pCVR + 预算 |
| 价值区分 | 一视同仁 | 区分高/低价值转化 | 优先高pCVR |
| 适用 KPI | CPA | ROAS/收入 | 转化量 |
| 回传要求 | 转化 | 转化+价值 | 转化 |
| 风险 | 目标过低无量 | 价值失真则失真 | 无目标易成本上升 |

### F.3 关键对比：Viewable CPM vs 品牌手动 CPM

| 对比项 | Viewable CPM | 手动 CPM (品牌) |
|-------|-------------|----------------|
| 优化指标 | 可见展示 (viewable impressions) | 曝光量 |
| 数据需求 | 中 (可见性回传) | 无 |
| 是否自动出价 | 是 | 否 |
| 目标 | 目标 vCPM 内最大化可见展示 | 固定出价 |
| 适用 | 品牌可见性优先 | 纯品牌曝光 |
| 补充 | 配合第三方可见性监测（Moat/DoubleVerify） | 简单直接 |

---

## 附录 G：常见指标与换算速查

在配置与监控出价策略时，经常需要在 CPM/CPC/CPA/ROAS 之间换算。下面给出速查公式。

```
CPM 与 CPC 换算 (已知 CTR):
  CPM = CPC × CTR × 1000
  CPC = CPM / (CTR × 1000)
  例: CPC=$1, CTR=1% → CPM = $1×0.01×1000 = $10

CPM 与 CPA 换算 (已知 CVR, 按展示口径):
  CPA = CPM / (CVR × 1000)
  CPM = CPA × CVR × 1000        ← 这正是 oCPM 公式

ROAS 与 CPA 换算 (已知转化价值 value):
  ROAS = value / CPA
  CPA  = value / ROAS
  例: value=$50, ROAS=5 → CPA=$10

pitch (万次) 与 single 换算:
  CPM = eCPM (有效千次展示成本)
  eCPM = 花费 / 曝光 × 1000
```
...
def unit_convert():
    ...
    cpm = 10.0
    ctr = 0.01   # 1%
    cpc = cpm / (ctr * 1000)     # = 1.0
    cvr = 0.02   # 2%
    cpa = cpm / (cvr * 1000)     # = 0.5 (假设每次转化)
    value = 50.0
    roas = value / cpa if cpa else 0
    print(f"CPM=${cpm} → CPC=${cpc:.2f}, CPA=${cpa:.2f}, ROAS={roas:.1f}x (value=${value})")
    # 呼应 oCPM: 目标CPA $0.5 × pCVR 0.02 × 1000 = $10 = CPM ✓
...
**运行输出**：
```
CPM=$10 → CPC=$1.00, CPA=$0.50, ROAS=100.0x (value=$50)
```

> 注：上例为纯数学换算说明 oCPM 公式方向；实际 DV360 中 CPA 与 CPM 的换算还受转化归因、频次、可见性等影响。

---

## 附录 H：Bid Surge 生命周期与合规提示

### H.1 Bid Surge 完整生命周期
...
│  │            │              │              │              │      │
│  ▼            ▼              ▼              ▼              ▼      │
│ 确认场景   设置幅度/窗口  启动 Surge    恢复基准出价    对比数据      │
│ 隔离效果单  +50%~+200%   监p/pacing   确认移除配置    复盘成本/量   │
│ 预算预留   设 bid ceiling 防超支       观察回落       沉淀经验     │
└───────────────────────────────────────────────────────────────┘
```

### H.2 常见 Surge 操作事故与预防

| 事故 | 原因 | 预防 |
|-----|------|------|
| Surge 忘了关 | 大促结束未移除配置 | 用附录 B 的"surge 残留检测"规则自动告警 |
| Surge 叠加效果单 | 误在效果 Line Item 上开 | 配置前核对 Line Item 类型/目标 |
| Surge 导致预算爆了 | 幅度大 + 预算小 | 预算预留 + 起设低幅度 |
| Surge 污染自动学习 | 与自动出价叠加 | 效果自动出价不使用 Surge |

### H.3 合规与政策提示

- 出价与 Surge 使用需遵守各交易所/广告平台政策，避免人为操纵拍卖（如循环出价、价格共谋等）风险；
- 对违规出价行为（远超合理估值、长期高于 floor 压量等）平台可能限制账户；
- 涉及敏感品类（金融、医疗、政治）的竞价，务必配合品牌安全与合规投放策略。
...
| 英文 | 全称/中文 | 说明 |
|------|----------|------|
| sCPM / Fixed CPM | 标准/固定千次展示 | 手动出价 |
| oCPM | 优化千次展示 | 自动优化出价的 CPM |
| tCPA | Target CPA | 目标每次转化费用 |
| tROAS | Target ROAS | 目标广告支出回报率 |
| pCVR | predicted CVR | 预测转化率 |
| pCTR | predicted CTR | 预测点击率 |
| vCPM / Viewable CPM | 可见千次展示 | 可见性优化 |
| PG / Programmatic Guaranteed | 程序化保量 | 固定价格保量 |
| PMP | Private Marketplace | 私有市场 |
| Bid Surge | 出价激增 | 临时出价增强 |
| Bid Floor | 底价 | 卖方最低出价 |
| Bid Shading | 出价遮罩 | 第一价格下压价 |
| Pacing | 投放节奏 | 预算平滑分配 |
| Learning Period | 学习期 | 模型收敛窗口 |
| Win Rate | 中标率 | 竞争力指标 |
| Delivery Rate | 交付率 | 视觉达程度 |
| Frequency Cap | 频次上限 | 单用户限制 |
| Floodlight | DV360 转化标签 | 转化/活动追踪 |
...
**每周一（复盘）**
- 用 `dv360_get_line_item_performance` 拉上周各 Line Item 的 CPA/ROAS/交付；
- 对比各出价策略（手动 vs 自动、tCPA vs tROAS）的 ROI，形成选型结论；
- 复盘上周 Surge/事件投放，沉淀经验。

**每周三（轻调优）**
- 对表现稳定达标且需扩量的 Line Item 适度收紧目标或加预算；
- 用 `dv360_list_recommendations` 评估是否有值得人工判断的出价建议；
- 检查 pacing 异常（`dv360_get_pacing_rate`）。

**每周五（体检）**
- 全账户出价策略健康审计；
- 确认无 Surge 残留、无策略误配、无回传异常；
- 规划下周事件/大促的 Surge 与预算预留。

**关键节拍**：**学习期内零改动** + **单变量调整** + **预留 3-7 天观察收敛**。背离这三个节拍，是绝大多数自动出价"越调越差"的根源。
...
1. 竞价策略是 DV360 连接 KPI 与拍卖的枢纽，选错策略代价会被生命周期放大。
2. 手动出价（CPM/CPC/CPV）可控但僵化；自动出价（Maximize/tCPA/tROAS/vCPM/oCPM）数据驱动，但依赖回传、学习期与一个稳定目标。
3. **Bid Surge 是叠加增强器，只适合品牌/冲刺，禁用于效果目标，且需与预算、频控、bid ceiling 协同。**
4. 拍卖机制影响一切：第一价格 + 自动出价（含 bid shading）通常优于手动；floor 过低目标会无量。
5. 自动出价内部是"预测（pCVR/pCTR）→ 目标驱动出价 → pacing → 校准反馈"闭环。
6. tCPA = 成本上限导向，tROAS = 回报导向（依赖价值回传），Maximize = 预算驱动最大转化。
7. PG 保量是"固定价格 + 保量算法 + 按时足量交付"，出价不再追求赢竞价而是匹配交付目标。
8. 用 API（`dv360_get_pacing_rate`、`dv360_get_line_item_performance`、`dv360_list_recommendations`）做数据驱动的持续监控与调优，是生产环境的关键。

### 进一步阅读（知识库内相关文档）

- 《DV360 平台架构与程序化购买深度解析》— 平台/账户/定向/创意/测量全景
- 《DV360 投放策略与优化深度实战》— 优化侧（含 2.3 竞价优化策略）
- 《DV360/DFP 竞价与投放架构深度》— 第四部分 oCPM 公式与 RTB 对接
- 《DV360 营销 API 深度解析》— API 整体使用
- 《DV360 测量与归因深度解析》— 转化回传与归因（自动出价的数据前提）
- 《DV360 创意与品牌安全深度解析》— 品牌场景与合规

> 本文档成文于 2026-08-14，由 Ryan 个人知识库整理，供团队内部学习与运维参考。涉及 DV360 具体字段/行为时以官方 API 文档与平台最新版本为准。
