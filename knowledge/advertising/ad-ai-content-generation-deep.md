...
1. **相关性**：生成的创意文案与产品描述/目标受众的语义相似度（BERTScore）
2. **多样性**：同一主题下多个生成的创意是否真正不同（BLEU/ROUGE 去重）
3. **品牌一致性**：是否包含正确的品牌关键词、tone of voice 符合规范
4. **转化率预测**：用历史 CTR/CVR 模型预估该创意的预期表现
5. **人工评分**：设计师对生成质量打分（1-5 分），作为强化学习的 Reward signal
