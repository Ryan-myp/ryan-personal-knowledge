...
跨平台投放中，很多团队反复踩同样的坑。这里把最高频的六大误区讲透，并给出"正确姿势"。
...
错误做法：
  四个平台各 25%，以为"广撒网"。
  结果：每个平台都在冷启动+学习期，没有一个跑过；
        数据分散，无法判断哪个有效；总 ROAS 反而下降。

正确姿势：
  ├── 按定位矩阵选"当前阶段最该上的平台"
  ├── 逐个加平台，验证增量后再加下一个（3.2）
  └── 预算按"目标×行业×地区"模型分配，不是均分
```
...
```
错误做法：
  只盯着 Google/Meta/TikTok 各自报表，谁报表好看就加谁预算。
  结果：三份报表都说"转化是我的"，合计 > 120%；
        预算被"归因膨胀"误导，真实增量被高估。

正确姿势：
  ├── 建立统一事件表 + 统一归因口径（2.7 / 4.5）
  ├── 用 MMM + 增量实验校准真实贡献（3.6）
  └── 用"增量 ROAS / 新客占比"做平台价值评估
```
...
```
错误做法：
  用单一 ROAS 排名分配预算，品牌平台(DV360)永远拿不到预算。
  结果：短期效果好看，但品牌资产流失；
        品牌搜索量下滑，长期效果平台也会触顶。

正确姿势：
  ├── 品牌平台用 Reach/VCR/Brand Lift 评估（不是 ROAS）
  ├── 双轨 KPI：效果 ROAS + 品牌健康度
  └── 品牌广告"滞后/溢出"价值纳入分配模型
```
...
```
错误做法：
  今天看 Meta 好，把 Google 预算砍一半给 Meta；
  后天看 TikTok 好，又大挪移。
  结果：每个平台都反复进入学习期，算法学不动，
        整体效果劣化。
...
```
错误做法：
  各平台独立投放，同一用户一天被 4 个平台触达 20 次。
  结果：CPM 上升、素材疲劳加速、用户体验差、预算浪费。

正确姿势：
  ├── 平台内频控 + 跨平台频控（CM360/ADH/自建）
  ├── 第一方 ID 打通，去重
  └── 监控"平均触达次数"，超过阈值自动降频
```
...
```
错误做法：
  3 人团队自营 DV360，每人学三个月还没跑通。
  结果：高人力成本，账号利用率低，效果差。

正确姿势：
  ├── 小团队 DV360 外包给代理商/Partner
  ├── 中团队配媒介经理再自营
  └── 大团队才独立成程序化团队
```
...
```
自查清单（每季度过一遍）：
□ 我是否在"逐平台验证增量"而非"盲目扩平台"？
□ 我的决策数据来自"统一事实源"还是"单平台报表"？
□ 我是否同时跟踪品牌健康度（品牌搜索量/声量）？
□ 我的预算调整是"周度小步"还是"频繁大挪移"？
□ 我是否做了跨平台频控/去重？
□ 我的平台运营方式是否匹配团队规模？
```
...
| 最大化点击 | ✅ | ✅ | — | — |
| CBO 类预算优化 | ✅ | ✅ | ✅ | 脚本化 |
| 全自动 Campaign | PMax | Advantage+ | Smart+ | — |
| 自动创意组合 | PMax | Advantage+ Creative | ACO | — |
| 动态素材优化 | ✅ | ✅ | ✅ | 有限 |
| 频控自动化 | ✅ | ✅ | ✅ | ✅ |
...
|------|:------:|:----:|:------:|:-----:|
| 站内转化追踪 | ✅ | ✅ | ✅ | 部分 |
| 服务端事件 | ✅ (GA4/Measurement) | ✅ (CAPI) | ✅ (Events API) | 部分 |
| 归因窗口配置 | ✅ 灵活 | ✅ | 较短 | ✅ |
| 离线数据回传 | ✅ | ✅ | ✅ | ✅ |
| 深度数据导出 | ✅ BigQuery | 部分 | 部分 | ✅ ADH |
| 增量实验支持 | ✅ | ✅ | 有限 | ✅ |
| MMM 数据友好度 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ |
...
| 竖版视频 | ✅ | ✅ | ✅ | ✅ |
| 互动/试玩 | 有限 | 有限 | 试玩广告 | 有限 |
| 达人共创 | — | Creator Collab | Spark Ads ✅ | — |
| 电商闭环 | Shopping | Shops | TikTok Shop | — |
| 动态创意 | ✅ | ✅ | ✅ | ✅ |
| 品牌特效/滤镜 | — | AR/特效 | Branded Effects | — |

---

## 附录 F：跨平台月度运营 SOP（时间表）

```
跨平台月度运营 SOP（可挂日历）

  第 1 周（复盘+规划）
  ├── 上月全渠道复盘（ROAS/CPA/增量/品牌）
  ├── 预算框架回顾与重分配建议
  ├── 上月 learnings 归档
  └── 本月素材/达人计划
...
决策工具 1：平台选型计分卡
  ...
  规则：总分最高者优先；组合时选"互补型"而非"同质型"
```

### G.2 预算分配工具

```
决策工具 2：预算分配模板（Excel/Sheets 字段）

  平台 | 目标类型 | 占比 | 月预算 | 7天ROAS | 增量ROAS
       | 新客占比 | 学习预算 | 上下限 | 决策依据 | 负责人
  ─────────────────────────────────────────────
  用 3.1.5 的 allocate_budget 脚本自动生成建议
```

### G.3 平台迁移工具

```
决策工具 3：迁移门槛检查（Go/No-Go）
...
决策工具 4：团队规模自评（选最接近项）
  ...
  1. 投放人数？  A.1~2  B.3~8  C.9+
  2. 月预算？    A.<$20K  B.$20K~$100K  C.$100K+
  3. 平台数？    A.1~2  B.3  C.4(含DV360)
  4. 数据能力？  A.托管工具  B.分析师  C.数据科学团队
  5. 是否品牌投放？ A.否  B.少量  C.是(DV360)

  计分：A=1 B=2 C=3，总分
  ├── 5~7 → 小团队打法（自动化+外包DV360）
  ├── 8~11 → 中团队打法（渠道Owner+统一归因）
  └── 12~15 → 大团队打法（专家专职+数据科学）
```
...
| 中文 | 英文 | 说明 |
|------|------|------|
| 效果最大化 | Performance Max (PMax) | Google 全自动广告系列 |
| 高意图 PMax | High-Intent PMax (H-iMax) | 聚焦高意图库存 |
| 全自动购物 | Advantage+ Shopping | Meta 电商全自动 |
| 达人广告 | Spark Ads | TikTok 达人原生推广 |
| 程序化直采 | Programmatic Direct | DV360 保量交易 |
| 可见度 | Viewability | 广告可见比例 |
| 视频完播率 | Video Completion Rate (VCR) | 视频观看完成比例 |
| 品牌提升 | Brand Lift | 品牌指标提升测量 |
| 增量测试 | Incrementality Test | 排除自然的增量验证 |
| 营销组合模型 | Marketing Mix Modeling (MMM) | 宏观媒体归因 |
| 多触点归因 | Multi-Touch Attribution (MTA) | 用户路径归因 |
| 学习期 | Learning Phase | 自动化算法学习阶段 |
| 频控 | Frequency Cap | 控制曝光次数 |
| 素材疲劳 | Creative Fatigue | 素材效果衰减 |

---

## 附录 I：跨平台投放 KPI 阈值参考（经验值）

> 以下为经验参考值，行业/阶段不同需调整，仅作基线。

| 指标 | 小团队/冷启动 | 成熟期 | 优秀 |
|------|:------------:|:------:|:----:|
| 全渠道 ROAS | 1.5~2.0x | 2.5~3.0x | 3.5x+ |
| 增量 ROAS | 1.2~1.5x | 1.5~2.0x | 2.5x+ |
| 新客占比 | — | 30~40% | 50%+ |
| 归因膨胀率 | — | ≤120% | ≤110% |
| 素材疲劳周期 | — | 3~6 周 | 6 周+（轮换好） |
| 平均触达/天 | — | 3~5 次 | 3 次以下 |
| 冷启动周期 | 2~6 周 | — | — |
| 单平台学习期 | 3~14 天 | — | — |
| 数据上报延迟 | <24h | <6h | <1h |
...
```
□ 用 G.1 计分卡跑一遍平台选型（现状复盘）
□ 检查 UTM 规范是否统一（抽查 10 条落地页链接）
□ 建立/更新 metrics_daily 表（至少覆盖当前平台）
□ 确认归因口径（写进团队 wiki）
□ 对当前主力平台做一次 ROAS 基线记录
```
...
- Google Ads Help / Google Marketing Platform
- Meta Business Help Center / Meta Marketing API 文档
- TikTok Ads Manager 帮助中心 / TikTok Business API 文档
- Display & Video 360 Help / DV360 API 文档
- Google Analytics 4 / BigQuery 文档

### K.2 本知识库相关文档索引

```
平台深度文档：
├── google-ads/   （Search/PMax/Display/API/优化）
├── meta-ads/     （Advantage+/定向/创意/API）
├── tiktok-ads/   （In-Feed/Spark/Shop/API）
├── dv360/        （架构/创意/测量/优化）
└── cross-channel-optimization/（预算分配/跨渠道）

关联专题：
├── ad-cross-platform-attribution-deep.md（跨平台归因）
├── ad-attribution-shapley-markov-deep.md（归因算法）
├── privacy-first-party-data.md（隐私第一方数据）
├── capi-deep-dive.md（服务端转化）
└── ad-data-pipeline-deep.md（数据管道）
```

---

*附录完毕。全文覆盖：平台定位矩阵、能力雷达图、预算分配、平台迁移、组织适配、排查手册、自测题与决策工具。*

*建议使用方式：作为团队跨平台战略的"顶层手册"，结合平台级深度文档（google-ads/、meta-ads/、tiktok-ads/、dv360/）与跨渠道专题文档交叉阅读；季度复盘时回到附录 J 的执行清单落地。*
