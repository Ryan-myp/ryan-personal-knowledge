...
|------|-------|-----------|---------|
| Meta | meta-marketing-api-expert | meta_client.py | 25（账户、层级资源、Lead Ads、报表与生命周期接口） |
| Google Ads | google-ads-api-expert | google_ads_client.py | 26（层级资源、Search Ad、Responsive Display Ad、关键词、Product Group、PMax、报表与生命周期接口） |
...
> 123 是当前四个 Capability 已实现的 Client 方法/业务 Tool 数量，不是 Meta、Google Ads、TikTok 或 DV360 官方 API 的完整接口总量。新增官方接口时，应在对应渠道 Client 增加固定方法，在 Capability 增加 Tool Schema/adapter，再由覆盖率审计和契约快照阻止漏注册或漂移。
