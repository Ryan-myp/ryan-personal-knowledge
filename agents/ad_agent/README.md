...
| Google Ads | google-ads-api-expert | google_ads_client.py | 57（层级资源、CampaignBudget、CampaignCriterion 定向、Conversion Action 生命周期、User List 生命周期与 Customer Match 哈希数据上传、BiddingStrategy 生命周期与优化参数、可复用文本/图片/YouTube/HTML5 Asset 创建、Search Ad、Responsive Display Ad、Video Ad、关键词、Product Group、PMax、报表与生命周期接口） |
