...
- 单 Agent + 多 Skills + Tools；平台 Capability 是可执行注册表的来源，当前共 152 个工具：Meta 35、Google Ads 41、TikTok 44、DV360 32。Capability 和按约定命名的 Provider Client 均自动发现，不依赖中心渠道/工具配置表；每个 Capability 还提供 Provider 方法覆盖率发布门禁。
