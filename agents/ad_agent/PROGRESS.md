...
- 单 Agent + 多 Skills + Tools；平台 Capability 是可执行注册表的来源，当前共 107 个工具：Meta 24、Google Ads 23、TikTok 40、DV360 20。Capability 和按约定命名的 Provider Client 均自动发现，不依赖中心渠道/工具配置表；每个 Capability 还提供 Provider 方法覆盖率发布门禁。
...
测试结果：
- 当前 `agents/ad_agent/tests/`：225 passed（API server contract 因本机 pydantic 架构依赖未收集）
...
| 单元测试 | ✅ | 非 API server contract 全量 225 个用例 |
