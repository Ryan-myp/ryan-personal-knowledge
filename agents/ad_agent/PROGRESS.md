...
- 单 Agent + 多 Skills + Tools；平台 Capability 是可执行注册表的来源，当前共 190 个工具：Meta 53、Google Ads 56、TikTok 49、DV360 32。Capability 和按约定命名的 Provider Client 均自动发现，不依赖中心渠道/工具配置表；每个 Capability 还提供 Provider 方法覆盖率发布门禁。
...
测试结果：
- 当前 `agents/ad_agent/tests/`：347 passed（本轮完整回归；另有 1 条本机依赖弃用 warning）。
...
| 单元测试 | ✅ | `agents/ad_agent/tests/`：347 passed |
