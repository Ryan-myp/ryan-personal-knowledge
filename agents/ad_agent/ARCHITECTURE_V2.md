...
## 三、工具清单（当前 Capability 共 191 个工具）
...
| **TikTok** | 49 | 账户、查询/创建 Campaign、Ad Group、Ad；Lead/App 广告；图片/视频 Asset Library 上传；转化、地域、设备、目录、应用、品牌安全查询；Spark Ads；受众 CRUD；生命周期；报表 |
...
当前 191 个 Tool 是四个 Capability 对其已实现 Client 方法的覆盖基线，不等于四个
...
| **Tools** | Capability/plugin 提供的具体可执行工具 | 按注册结果动态统计（当前基线 191 个） |
