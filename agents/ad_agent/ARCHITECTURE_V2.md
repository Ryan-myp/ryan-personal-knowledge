...
## 三、工具清单（当前 Capability 共 129 个工具）

| 平台 | 工具数量 | 工具列表 |
|------|---------|---------|
| **Meta** | 28 | 账户、查询/创建 Campaign、Ad Set、Ad、Lead Ads、Catalog Ads、Creative；商品目录/商品集；受众；Boost；生命周期；报表 |
| **Google Ads** | 27 | 查询/创建 Campaign、Ad Group、Ad、关键词、Search Ad、Responsive Display Ad、Video Ad、Product Group、PMax Asset Group；生命周期；报表 |
...
当前 129 个 Tool 是四个 Capability 对其已实现 Client 方法的覆盖基线，不等于四个
...
| **Tools** | Capability/plugin 提供的具体可执行工具 | 按注册结果动态统计（当前基线 129 个） |
