...
## 三、工具清单（当前 Capability 共 147 个工具）
...
| **TikTok** | 43 | 账户、查询/创建 Campaign、Ad Group、Ad；Lead/App 广告；素材；转化、地域、设备、目录、应用、品牌安全查询；Spark Ads；受众创建与查询；生命周期；报表 |
...
当前 147 个 Tool 是四个 Capability 对其已实现 Client 方法的覆盖基线，不等于四个
...
| **Tools** | Capability/plugin 提供的具体可执行工具 | 按注册结果动态统计（当前基线 147 个） |
