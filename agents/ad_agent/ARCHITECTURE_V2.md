...
## 三、工具清单（当前 Capability 共 124 个工具）

| 平台 | 工具数量 | 工具列表 |
|------|---------|---------|
| **Meta** | 25 | 账户、查询/创建 Campaign、Ad Set、Ad、Lead Ads、Creative；受众；Boost；生命周期；报表 |
...
当前 124 个 Tool 是四个 Capability 对其已实现 Client 方法的覆盖基线，不等于四个
...
| **Tools** | Capability/plugin 提供的具体可执行工具 | 按注册结果动态统计（当前基线 124 个） |
