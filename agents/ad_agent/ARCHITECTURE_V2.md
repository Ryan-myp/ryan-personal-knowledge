...
## 三、工具清单（当前 Capability 共 183 个工具）
...
| **Google Ads** | 49 | 查询/创建 Campaign、CampaignBudget、CampaignCriterion 定向、Conversion Action 生命周期、出价策略/User List 查询、Ad Group、Ad、关键词、Search Ad、Responsive Display Ad、Video Ad、Product Group、PMax Asset Group；生命周期；报表 |
...
当前 183 个 Tool 是四个 Capability 对其已实现 Client 方法的覆盖基线，不等于四个
...
| **Tools** | Capability/plugin 提供的具体可执行工具 | 按注册结果动态统计（当前基线 183 个） |
