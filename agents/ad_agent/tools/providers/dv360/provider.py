"""
tools/providers/dv360/provider.py - DV360 Tool Source 定义
"""
import logging
...
from ..provider_base import BaseProviderToolSource, CampaignUpdateHandler
...
class DV360ToolSource(BaseProviderToolSource):
    platform_name = "dv360"
    provider_client_class = DV360APIClient
    tool_source_version = "1.2.0"
    ...
    def get_creation_blueprints(self) -> list:
        ...
        this Tool Source does not expose a Campaign create Tool. The guided
    ...
    def _extended_provider_tools(self, client):
        ...
        tools = [
            ...
            method_tool(
                ...
                properties={"advertiser_id": {"type": "string"}, "line_item_id": {"type": "string"},
                            "targeting_type": {"type": "string"},
                            "assigned_targeting_option": {
                                "type": "object",
                                "description": "DV360 TargetingOption assignment payload",
                                "additionalProperties": True,
                                "presentation": "advanced_json",
                                "manual_entry": {
                                    "title": "DV360 定向绑定高级配置",
                                    "instructions": "DV360 的 targetingOptionDetails 由 targeting_type 决定；定向深度建设暂缓，请使用当前 DV360 返回或已审核的对象 payload。",
                                    "source": "dv360_assigned_targeting_option_payload",
                                },
                            }},
                ...
                requires=["targeting_type", "assigned_targeting_option"],
                argument_builder=lambda ctx, data: ((account(ctx, data), data["line_item_id"], data["targeting_type"],
            ...
            method_tool(
                ...
                properties={"advertiser_id": {"type": "string"},
                            "report": {
                                "type": "object",
                                "description": "DV360 asynchronous report definition",
                                "additionalProperties": True,
                                "presentation": "advanced_json",
                                "manual_entry": {
                                    "title": "DV360 报表定义高级配置",
                                    "instructions": "报表 dimensions、metricColumns、时间范围和过滤器会随 DV360 报表版本变化；请使用已审核的报表定义。",
                                    "source": "dv360_report_definition_payload",
                                },
                            }},
    ...
    def register_tools(self) -> list[tuple[ToolDefinition, ToolHandler]]:
        ...
        tools.append((ToolDefinition(
            ...
            intent_aliases=["列出 DV360 广告系列", "查询 DV360 campaign 列表"],
        ...
        tools.append((ToolDefinition(
            ...
            intent_aliases=["查询 DV360 campaign 详情", "查看 DV360 campaign 详情", "查询 DV360 广告系列详情"],
            risk_level=RiskLevel.LOW,
            effect_class=ToolEffect.READ,
            replay_policy=ReplayPolicy.SAFE,
            traits=["read", "campaign"],
        ...
        tools.append((ToolDefinition(
            ...
            intent_aliases=["查询 DV360 line item 详情", "查看 DV360 line item 详情"],
            risk_level=RiskLevel.LOW,
            effect_class=ToolEffect.READ,
            replay_policy=ReplayPolicy.SAFE,
            traits=["read", "line_item"],
        ...
        tools.append((ToolDefinition(
            ...
            intent_aliases=[
                "下载 DV360 报表", "查询 DV360 报表",
                "下载 DV360 line item 报表", "查询 DV360 line item 报表",
                "下载 DV360",
            ],
            risk_level=RiskLevel.LOW,
            effect_class=ToolEffect.READ,
        ...
        for resource_type, resource_id in [("campaign", "campaign_id"), ("io", "io_id"), ("line_item", "line_item_id")]:
            ...
            tools.append((ToolDefinition(
                ...
                parent_resource_type={"line_item": "io"}.get(resource_type),
                intent_types={
                    "campaign": [
...
def create_dv360_tool_source(api_client: Optional[DV360APIClient] = None) -> DV360ToolSource:
    cap = DV360ToolSource()
