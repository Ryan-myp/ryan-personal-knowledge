"""Canonical Tool Source construction for the ad-agent runtime.

The runtime used to keep a second namespace-to-module map in addition to the
public Tool Source factories.  That made dynamic Skill loading subtly
different from the CLI/API registration path.  This module is the single
    construction seam for namespace Tool Sources; it only constructs Python
...
def _discover_noncanonical_module(canonical: str):
    ...
    for module_info in pkgutil.iter_modules(getattr(package, "__path__", ())):
        if not module_info.ispkg or module_info.name.startswith("_"):
            continue
        module_name = f"{__package__}.{module_info.name}.provider"
        try:
        ...
        factories = sorted(
            ...
            and name.endswith("_tool_source")
            and callable(value)
        )
        if factories:
            return factories[0]
...
def discover_tool_source_factory(namespace: str):
    """Find a Tool Source factory by package convention.
    ...
    whose Tool Source declares the requested namespace identity.
    """
    canonical = normalize_namespace(namespace)
    slug = _module_slug(canonical)
    module_name = f"{__package__}.{slug}.provider"
    try:
        module = importlib.import_module(module_name)
    except ModuleNotFoundError as exc:
    ...
    candidates = [f"create_{slug}_tool_source"]
...
def create_tool_source(namespace: str, api_client: Optional[Any] = None):
    """Create a namespace Tool Source through its public factory.

    ``api_client`` is injected as-is. Tool Source construction is
    deliberately side-effect free; a handler may perform network I/O only when Runtime
    executes a read tool or an explicitly approved live write.
    """
    canonical = normalize_namespace(namespace)
    factory = discover_tool_source_factory(canonical)
    if not callable(factory):
        raise ValueError(
            f"Unsupported ad namespace '{namespace}'. "
            "Add tools/providers/<namespace>/provider.py with a "
            "create_<namespace>_tool_source factory."
        )
