...
Provider Tool Sources own the endpoint catalog.  This module only removes the
...
from .provider_base import ProviderMethodHandler
...
def method_tool(
    *,
    ...
    requires_any_of: Optional[Iterable[Iterable[str]]] = None,
    requires_exactly_one_of: Optional[Iterable[Iterable[str]]] = None,
    ...
    definition = ToolDefinition(
        ...
        input_schema=ToolSchema(
            ...
            requires_any_of=[list(group) for group in (requires_any_of or [])],
            requires_exactly_one_of=[
                list(group) for group in (requires_exactly_one_of or [])
            ],
...
def bind_provider_method(
    ...
    """Bind a Tool Source's injected client without rebuilding its contract."""
